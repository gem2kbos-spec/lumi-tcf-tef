...
export async function buildDetailedAttemptAnalysis(question, selected) {
  ...
  try {
    const output = await completeAi({
      instructions: "你是TCF法语考试中文精讲老师。必须针对当前题逐项分析，禁止使用‘不合适’‘不符合语境’‘需要检查’等空话。每个选项必须说明该词或结构的真实常见用法、本题中为何成立或不成立，并给一个毫无争议、自然、常用的法语正确例句。错误选项的例句应展示该词本身的标准用法，不得继续使用本题中已判错的搭配，不得编造冷僻搭配。所有说明使用详细、准确、易懂的中文；法语只出现在原句、术语和例句中。不得修改题目或正确答案。",
