...
let commentKind = "question"; let replyToComment = null; let commentSort = "newest";
...
function renderCommentCard(comment, replies = []) { const replyCards = replies.length ? `<div class="comment-replies">${replies.map((reply) => renderCommentCard(reply)).join("")}</div>` : ""; return `<article class="comment-item ${escapeHtml(comment.kind || "question")} ${comment.author.role === "admin" ? "admin" : ""}"><div class="comment-author"><span class="comment-avatar">${escapeHtml(commentAvatar(comment.author.name))}</span><div class="comment-meta"><strong>${escapeHtml(comment.author.name)}</strong>${comment.author.role === "admin" ? '<span class="comment-admin">管理员</span>' : ""}<span class="comment-kind ${escapeHtml(comment.kind || "question")}">${commentKindLabel(comment.kind)}</span>${comment.kind === "report" ? `<span class="comment-report-status">${reportLabel(comment.reportStatus)}</span>` : ""}<time>${new Date(comment.createdAt).toLocaleString("zh-CN", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" })}</time></div></div><p>${escapeHtml(comment.content)}</p><div class="comment-footer"><div><button class="comment-like${comment.liked ? " liked" : ""}" data-comment-like="${escapeHtml(comment.id)}">${comment.liked ? "♥" : "♡"} ${comment.likes || "赞"}</button><button class="comment-reply" data-comment-reply="${escapeHtml(comment.id)}">回复</button></div>${comment.canDelete ? `<button class="comment-delete" data-comment-id="${escapeHtml(comment.id)}">删除</button>` : ""}</div>${replyCards}</article>`; }
async function loadQuestionComments() {
  ...
  try {
    ...
    let roots = payload.comments.filter((comment) => !comment.parentId); if (commentSort === "reports") roots = roots.filter((comment) => comment.kind === "report"); else if (commentSort === "popular") roots.sort((a, b) => (b.likes - a.likes) || b.createdAt.localeCompare(a.createdAt)); else roots.sort((a, b) => b.createdAt.localeCompare(a.createdAt)); const replies = new Map(roots.map((comment) => [comment.id, []])); payload.comments.filter((comment) => comment.parentId && replies.has(comment.parentId)).forEach((comment) => replies.get(comment.parentId).push(comment));
    list.innerHTML = roots.length ? roots.map((comment) => renderCommentCard(comment, replies.get(comment.id))).join("") : `<p>${commentSort === "reports" ? "这道题暂时没有纠错提交。" : "还没有讨论。第一个提出具体问题的人，通常收获最多。"}</p>`;
...
function render() {
  ...
  $("#comment-content").value = ""; setReplyTo(); setCommentKind("question"); $("#question-comments").hidden = true; $(".comment-body").hidden = true; $("#toggle-comments b").textContent = "展开"; $("#comment-count").textContent = "按需查看";
