...
import grammar from "../server/mock-grammar-batch-005.json" with { type: "json" };
import reading from "../server/mock-reading-batch-005.json" with { type: "json" };
...
function verify(items, type, prefix) {
  ...
  items.forEach((item, index) => {
    assert.equal(item.id, `${prefix}-${String(index + 201).padStart(3, "0")}`);
    assert.equal(item.type, type);
    assert.equal(item.exam, "tcf");
    assert.equal(item.source, "mock");
    assert.equal(item.batch, 5);
    ...
    assert.equal(item.skill, item.skill.charAt(0).toLocaleLowerCase("fr") + item.skill.slice(1));
...
test("fifth TCF grammar batch contains 50 audited A2-B1 questions", () => verify(grammar, "grammar", "mock-msl"));
test("fifth TCF reading batch contains 50 audited A2-B1 questions", () => verify(reading, "reading", "mock-reading"));

test("fifth batch remains unique across the complete bank", () => {
