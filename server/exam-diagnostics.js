...
function lexicalDiagnostic(attempt, text, answer) {
  ...
  return {
    ...
    examAbility: `TCF · ${inPassage ? "篇章词汇理解" : "词汇/语域与情境适切"}`,
...
function grammarDiagnostic(attempt, text, answer) {
  ...
  if (/\bdepuis\b/.test(text)) return {
    id: "time:depuis-present-continuity", examAbility: "TCF · 根据语境选择正确表述",
  ...
  if (/\b(aller|vais|vas|va|allons|allez|vont)\b/.test(text) && /\b(infinitif|demain|bientot|ce soir|la semaine prochaine)\b/.test(text)) return {
    id: "tense:futur-proche", examAbility: "TCF · 正确语法表述",
  ...
  if (skill === "connectors" || skill === "connecteurs") return {
    id: `connector:${normalized(answer) || "logic"}`, examAbility: `TCF · ${attempt.question?.passage ? "篇章衔接" : "等义表达与正确表述"}`,
  ...
  return {
    id: `grammar:${skill}:${normalized(title)}`, examAbility: "TCF · 正确语法表述",
