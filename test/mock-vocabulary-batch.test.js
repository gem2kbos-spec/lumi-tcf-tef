...
test("active bank has 1182 unique non-listening questions", async () => {
  const imported = await loadImportedQuestions(); const all = [...questionBank, ...imported];
  assert.equal(all.length, 1182);
  ...
  assert.deepEqual(Object.fromEntries(["grammar", "vocabulary", "reading"].map((type) => [type, all.filter((question) => question.type === type).length])), { grammar: 471, vocabulary: 302, reading: 409 });
});
